...
**后续跟踪（2026-07-12）：** ShipyardPanel（572→248 行）与 CharacterPanel（386→126 行）已按方案 A 分解。拆出 9 个子组件文件，构建/lint/测试全部通过。**✅ 确认关闭。**
---
...
**后续跟踪（2026-07-12）：** 采用方案 B 修复——HarborDashboard 状态栏 "舱容 N/M" 改为指向 `/fleet` 的链接，利用 FleetPanel 既有 cargo 明细表。**✅ 确认关闭。**
...
除先前关闭的组件分解和 cargo 路由外，UI 打磨还需关注：
- 导航栏缺少部分高频入口（副本、成就、图鉴、任务、NPC）
- 部分页面首次渲染无 loading 边界（白屏）
- 无全局 error 边界兜底

> 原审计条目包含「移动端适配检查」。经项目定位复盘（2026-07-12），目标平台决策为「PC 主目标 + 移动端浏览器基线兼容」，移动端深度适配不作为当前阶段目标，移动端相关条目在本节移除。

### 修复方案分析

1. **导航栏补全入口**：利用桌面端充裕的横向空间，将副本 / 成就 / 图鉴 / 人物 / 任务加入导航栏。nav 容器加 `flex-wrap` 为极端窄屏做基线保护。
