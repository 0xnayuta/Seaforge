...
> **状态：✅ 全部完成（Phase 3.0 – 3.4）**
> 所有子阶段均已实现并通过验证：`bun run build` ✓ `bun run lint` ✓ `bun run test` 357 pass ✓
...
- [x] 人物属性系统：5 核心属性 + 8 面板数值计算正确，软上限生效
- [x] 升级加点：每级 3 点分配正确影响面板
- [x] 物品系统：inventory 重构完成，ItemInstance 支持耐久/强化/词缀预留
- [x] 装备栏：主手武器 + 铠甲 + 饰品 ×2 可穿戴/卸下，属性正确叠加
- [x] 初始装备池 ≥ 15 件
- [x] 回合制战斗：伤害/暴击/闪避/行动顺序计算正确
- [x] 接舷战：舰队战败后可选人物战斗，结果正确反馈
- [x] NPC 系统：对话/送礼/好感度/招募流程完整
- [x] 船长招募直接影响 maxShips
- [x] 任务系统：好感度门槛 + 物品奖励完整
- [x] 至少 5 个可接受任务
- [x] 至少 2 个可招募 NPC 船长
...
- [x] 人物属性 Build 存在多样性：同一等级不同配点产生可感知的战斗差异
- [x] 装备存在显著品质梯度（普通→良→优秀→稀有→传说）
- [x] 回合制战斗有策略深度（不单纯是数值对撞）
- [x] NPC 好感度有实质影响（解锁专属任务/折扣/战斗加成）
- [x] UI 展示人物面板、背包、装备栏、战斗界面
